...
class LokiDataCollector:
    ...
    def collect_raw_logs(
        ...
        if job_id and collection_name:
            ...
            search_patterns.extend([f'|~ "{job_id}"', '|~ "import"'])
        elif collection_name:
            search_patterns.extend([f'|~ "{collection_name}"', '|~ "import"'])
        else:
        ...
        for i, pattern in enumerate(search_patterns):
            try:
                ...
                self.logger.debug(
                    f"Executing Loki query {i + 1}/{len(search_patterns)}: {query}"
                )
                ...
                self.logger.warning(
                    f"Failed to execute query with pattern '{pattern}': {e}"
                )
        ...
        self.logger.info(
            f"Collected {len(unique_logs)} unique logs from {raw_logs['metadata']['successful_queries']} successful queries"
        )
    ...
    def _query_loki(
        ...
        params = {
            "query": query,
            "start": int(
                start_time.timestamp() * 1_000_000_000
